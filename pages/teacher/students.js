...
const Students = () => {
    ...
    const filteredStudents = students?.filter(d => `${d?.first_name} ${d?.last_name}`.toLowerCase().includes(search.toLowerCase()))
    console.log('filteredStudents', filteredStudents)

    
    // if students is null means that fetching did not finish yet; load the skeleton first
    if(!students) {
                        ...
                        {loading ? <HistoryAwardSkeleton /> : (
                            <>
                                ...
                                {history?.docs?.length > 0 ? (
                                    <div className="flex justify-center text-center border-t border-gray-200 p-4 pb-1 sticky bottom-0 bg-white">
                                        <Pagination
                                            page={history?.page || 1}
